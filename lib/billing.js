...
// 運営者自身のアカウント。トライアルの対象にせず、案内も出さない。
// 自分のアプリを自分で使うのに登録を求められるのはおかしいため。
// 環境変数 OWNER_EMAILS に「,」区切りで書けば差し替えられる
// （大文字小文字と前後の空白は無視する）
// 既定の運営者。環境変数を入れ忘れても効くように、ここに置いてある。
// OWNER_EMAILS を設定した場合は、そちらで完全に置き換わる
const DEFAULT_OWNER_EMAILS = 'gachidai@gmail.com';

const OWNER_EMAILS = new Set(
  String(process.env.OWNER_EMAILS || DEFAULT_OWNER_EMAILS)
    .split(',')
...
function decide(row, now = new Date()) {
  // 運営者自身は、トライアルも案内もなしで全機能を使える
  if (isOwner(row.email)) return { access: true, aiPlan: 'premium', state: 'owner' };
...
async function loadRow(userId) {
  const u = await query(`SELECT plan, trial_ends_at, email FROM users WHERE id = $1`, [userId]);
  if (!u.rows[0]) return { plan: 'free', trialEndsAt: null, email: null, subscription: null };
