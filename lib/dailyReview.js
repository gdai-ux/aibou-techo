...
// 話し方。きびしさ（tone）とは別の軸にしてある。
// 同じ「鬼」でも、人のことばで詰めるのか、ねこのことばで詰めるのかを選べる
const REVIEW_SPEECHES = {
  normal: { label: 'ふつう', prompt: '' },
  cat: {
    label: 'ねこ',
    prompt: `話し方: ねこ。
語尾を「〜にゃ」「〜だにゃ」「〜にゃん」などに変える。一人称は「オレ」ではなく
「ボク」や「わし」など、きびしさに合う範囲でよい。
ただし、語尾を変えるだけで、中身（指摘の鋭さ・具体的な一手）は一切ゆるめないこと。
きびしい口調なら、ねこのことばのまま容赦なく詰める（例:「まだ全然足りてないにゃ。
いまから20分歩いてくるにゃ」）。全部の文を「にゃ」にすると読みにくいので、
2文に1回くらいの割合で自然に混ぜる。`,
  },
};
const DEFAULT_SPEECH = 'normal';
...
function buildSystemPrompt(tone, speech) {
  const t = REVIEW_TONES[tone] || REVIEW_TONES[DEFAULT_TONE];
  const sp = REVIEW_SPEECHES[speech] || REVIEW_SPEECHES[DEFAULT_SPEECH];
...
function summarizeDay(day) {
  ...
  if (day.sleep && day.sleep.bedtime) {
    ...
    // 眠りの質は本人の感覚。時間だけでは分からない「長く寝たのに浅い」日に
    // 相棒が触れられるよう、あれば一緒に渡す
    const quality = day.sleep.quality ? `　眠りの質: ${day.sleep.quality}` : '';
    lines.push(`睡眠: ${day.sleep.bedtime}就寝 → ${day.sleep.wake}起床${duration}${quality}`);
  }
...
function summarizeRecent(days, todayStr) {
  ...
  const rows = past.map((d) => {
    ...
    if (d.sleep && d.sleep.totalMinutes) {
      const q = d.sleep.quality ? `・${d.sleep.quality}` : '';
      parts.push(`睡眠${Math.floor(d.sleep.totalMinutes / 60)}時間${d.sleep.totalMinutes % 60}分${q}`);
    }
...
async function generateDailyReview(apiKey, day, tone, opts = {}) {
  ...
  const resp = await fetch(OPENAI_CHAT_URL, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.9,
      messages: [
        { role: 'system', content: buildSystemPrompt(tone, opts.speech) },
...
module.exports = { generateDailyReview, REVIEW_TONES, REVIEW_SPEECHES, DEFAULT_TONE, DEFAULT_SPEECH, summarizeDay, summarizeRecent, FOCUS_ANGLES, OPENING_STYLES };
