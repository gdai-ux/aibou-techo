...
const META_LABELS = { memo: 'メモ', sleep: '睡眠', nap: '昼寝', exercise: '運動', condition: '体調' };
// 昼寝の長さの上限（分）。これを超えるものは夜の睡眠として記録してもらう
const NAP_MAX_MINUTES = 720;
const MEAL_TYPES = ['朝食', '昼食', '夕食', '間食', '飲み物'];
const CONDITION_LEVELS = ['絶好調', '良い', '普通', '悪い', '最悪'];
// 眠りの質。時間だけでは「長く寝たのに疲れが取れない」日が見えないので、
// 本人の感覚を5段階で残す。任意なので、入れなくても睡眠は記録できる
const SLEEP_QUALITIES = ['ぐっすり', '良い', '普通', '浅い', '眠れず'];
...
function buildMetaText(category, payload) {
  ...
  if (category === 'sleep') {
    ...
    if (payload.quality && !SLEEP_QUALITIES.includes(payload.quality)) {
      throw new Error(`quality must be one of ${SLEEP_QUALITIES.join(', ')}`);
    }
    const { hours, minutes } = calcSleepDuration(payload.bedtime, payload.wake);
    const base = `${payload.bedtime}就寝、${payload.wake}起床（${hours}時間${minutes}分）`;
    return payload.quality ? `${base} 質：${payload.quality}` : base;
  }
  if (category === 'nap') {
    // 昼寝は「何分寝たか」で持つ。夜の睡眠と違って就寝・起床の時刻より
    // 長さの方が知りたい情報で、入力も速いため
    const minutes = Number(payload.minutes);
    if (!Number.isInteger(minutes) || minutes <= 0 || minutes > NAP_MAX_MINUTES) {
      throw new Error(`minutes must be an integer between 1 and ${NAP_MAX_MINUTES}`);
    }
    return payload.time ? `${payload.time} ${minutes}分` : `${minutes}分`;
  }
