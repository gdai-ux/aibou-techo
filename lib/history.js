...
async function fetchHistory({ token, pageId, limitDays = 30 }) {
  ...
  for (const b of children) {
    if (b.type === 'heading_2') {
      ...
      current = { dateStr: m[1], weekday: m[2], sleep: null, exercise: [], condition: [], memo: [], meals: [], review: null, bedtimeCarry: null, dataEditedAt: null };
      days.push(current);
      currentMeal = null;
      continue;
    }
...
function applyBedtimeCarry(days) {
  for (const day of days.slice()) {
    ...
    if (!prevDay) {
      prevDay = { dateStr: prev.dateStr, weekday: prev.weekday, sleep: null, exercise: [], condition: [], memo: [], meals: [], review: null, bedtimeCarry: null, dataEditedAt: null };
