...
const CATEGORY_LABELS = { memo: 'メモ', exercise: '運動', meal: '食事', sleep: '睡眠', nap: '昼寝', condition: '体調', review: 'ふりかえり' };
...
function entryProperties(category, payload, dateStr) {
  ...
  if (category === 'sleep') {
    ...
    // 眠りの質は睡眠が使っていない「補足」に入れる（列を増やさずに済む）
    props['補足'] = { rich_text: rt(payload.quality || '') };
  ...
  } else if (category === 'nap') {
    // 長さは「補足」に分数として持つ（就寝・起床の欄は使わない）
    props['時刻'] = { rich_text: rt(payload.time || '') };
    props['補足'] = { rich_text: rt(String(payload.minutes)) };
  } else {
  ...
  for (const row of rows) {
    ...
    if (category === 'memo') {
      ...
      if (bedtime && wake) {
        ...
        day.sleep = { blockId: row.id, bedtime, wake, hours, minutes, totalMinutes: hours * 60 + minutes, quality: readRt(p['補足']) || '' };
      } else {
        day.sleep = { blockId: row.id, raw: readRt(p['内容']) };
      }
    } else if (category === 'nap') {
      day.nap.push({ blockId: row.id, time, minutes: Number(readRt(p['補足'])) || 0 });
...
async function migrateDay(token, dbId, day) {
  ...
  if (day.sleep && day.sleep.bedtime && day.sleep.wake) {
    await add('sleep', { bedtime: day.sleep.bedtime, wake: day.sleep.wake, quality: day.sleep.quality || '' });
  }
