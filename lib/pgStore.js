...
const CATEGORIES = ['sleep', 'nap', 'exercise', 'memo', 'condition', 'meal', 'review'];
...
function normalizePayload(category, payload) {
  ...
  if (category === 'sleep') return { bedtime: p.bedtime, wake: p.wake, quality: p.quality || '' };
  if (category === 'nap') return { time: (p.time || '').trim(), minutes: Number(p.minutes) };
...
// Stripeを設定している環境でだけ、新規登録に無料トライアルの終了日時を付ける
// （ON CONFLICTでは更新しないので、既存の利用者のtrial_ends_atは変わらない）
async function ensureUser(id, email) {
  const billing = require('./billing');
  const trialEndsAt = billing.billingEnabled() ? billing.newTrialEndsAt() : null;
  await query(
    `INSERT INTO users (id, email, trial_ends_at) VALUES ($1, $2, $3)
     ON CONFLICT (id) DO UPDATE SET email = EXCLUDED.email`,
    [id, email, trialEndsAt]
  );
}
...
function emptyDay(dateStr) {
  ...
  return { dateStr: info.dateStr, weekday: info.weekday, sleep: null, nap: [], exercise: [], condition: [], memo: [], meals: [], review: null, bedtimeCarry: null, dataEditedAt: null };
}
  ...
  for (const row of r.rows) {
    ...
    if (row.category === 'memo' || row.category === 'exercise') {
      ...
      if (p.bedtime && p.wake) {
        ...
        day.sleep = { blockId: row.id, bedtime: p.bedtime, wake: p.wake, hours, minutes, totalMinutes: hours * 60 + minutes, quality: p.quality || '' };
      } else {
        day.sleep = { blockId: row.id, raw: '' };
      }
    } else if (row.category === 'nap') {
      day.nap.push({ blockId: row.id, time: orNull(p.time), minutes: Number(p.minutes) || 0 });
