...
const { REVIEW_TONES, REVIEW_SPEECHES, DEFAULT_TONE, DEFAULT_SPEECH, summarizeDay } = require('./dailyReview');
...
function buildSystemPrompt({ tone, name, bio, speech }) {
  const t = REVIEW_TONES[tone] || REVIEW_TONES[DEFAULT_TONE];
  // 話し方（ねこなら「〜にゃ」）。ふりかえりと同じものを使い、
  // 画面によって話し方が変わらないようにする
...
async function chatWithTrainer(apiKey, { tone, name, bio, speech, context, history, message }) {
  const systemContent = context
    ? `${buildSystemPrompt({ tone, name, bio, speech })}\n\n今の記録:\n${context}`
    : buildSystemPrompt({ tone, name, bio, speech });
  const messages = [
